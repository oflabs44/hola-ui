...
3a. **`color` prop is allowed ONLY where Catalyst proves it pays for itself.** Default: no color prop, follow shadcn. Add it when the component's primary value IS being recolored (badges, sometimes alerts, never buttons). Use Catalyst's 18-colour palette verbatim: `zinc, red, orange, amber, yellow, lime, green, emerald, teal, cyan, sky, blue, indigo, violet, purple, fuchsia, pink, rose`. The semantic `destructive` variant always ignores `color` — its red is the meaning, not the colour. See `registry/hola/ui/badge.tsx` for the reference implementation: separate color-style tables (`tintedBg`, `solidStyle`, `textColor`, `borderColor`) resolved in the component function, since the variant×color cross product is too sparse for clean `compoundVariants`.
